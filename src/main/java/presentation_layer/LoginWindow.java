...
public class LoginWindow  extends ConsoleView{
	...
	public void receiveLogin(Scanner scanner) {
		...
		while (loginTry) {
			...
			Login login = null;
			try {
				login = LoginMapper.findByUsernamePasswordCombination(username, password);

				if (login != null) {
					if (login.getUser_role().equals("admin")) {
						AdminWindow adminWindow = new AdminWindow();
						adminWindow.loginCorrect(login);
					} else if (login.getUser_role().equals("client")) {
						ClientWindow clientWindow = new ClientWindow();
						clientWindow.loginCorrect(login);
					} else {
						System.out.println(
