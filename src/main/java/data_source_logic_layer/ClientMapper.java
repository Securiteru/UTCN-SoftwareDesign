...
public class ClientMapper extends DataMapper{
	...
	public synchronized Client findByClientId(int client_id_to_search) throws DataMapperException {
		try {
			...
			throw new DataMapperException("Error occurred while reading Client from the data source.", e);
		}
	}

	public synchronized void insertFixed(Client client) throws DataMapperException {
		try {
			Connection db = conexiune.connection;
			String statement = "INSERT INTO `client_table` (`full_name`, `address`, `CNP`, `login_id`,`client_id`) VALUES (?, ?, ?, ?,?)";
			PreparedStatement dbStatement = db.prepareStatement(statement);
//			dbStatement.setInt(1, client.getClient_id());
			...
			dbStatement.setInt(5, client.getClient_id());
			dbStatement.executeUpdate();
		} catch (SQLException e) {
			throw new DataMapperException("Error occurred while inserting Client into the data source.", e);
		}
	}
	...
	public synchronized void update(Client client) throws DataMapperException {
		try {
			...
			throw new DataMapperException("Error occurred while reading Clients from the data source.", e);
		}
	}
	public synchronized void insert(Client client) throws DataMapperException {
		try {
			Connection db = conexiune.connection;
			String statement = "INSERT INTO `client_table` (`full_name`, `address`, `CNP`, `login_id`) VALUES (?, ?, ?, ?)";
			PreparedStatement dbStatement = db.prepareStatement(statement);
//			dbStatement.setInt(1, client.getClient_id());
			dbStatement.setString(1, client.getFull_name());
			dbStatement.setString(2, client.getAddress());
	...
	public synchronized void delete(Client client) throws DataMapperException {
		try {
			...
			throw new DataMapperException("Error occurred while deleting Client from the data source.", e);
		}
	}
	...
	public synchronized List<Client> getAllClients() throws DataMapperException {
		...
		try {
			Connection db = conexiune.connection;
			String statement = "SELECT `client_id`,`full_name`, `address`, `CNP`, `login_id` FROM `client_table`";
			PreparedStatement dbStatement = db.prepareStatement(statement);
			ResultSet rs = dbStatement.executeQuery();
			while(rs.next()) {
				String full_name = rs.getString("full_name");
			...
			throw new DataMapperException("Error occurred while reading all Client from the data source.", e);
		}
		return clientList;
	}
